---
title       : Import and export Excel file
description : "This chapter covers all the useful <code>readxl</code> functions that read Excel data into R."

--- type:NormalExercise lang:r xp:100 skills:1 key:086558b449
## Reading a .xlsx file

Welcome to this mini-course on the `readxl` package!  In this chapter, you’ll be exposed to many of the great features this package has to offer.

Let’s get started right away with some basic importing features.  Your first task will be to master the use of the `read_excel()` function.  There are many arguments available, but the only required argument is `path`, a path to a XLS/XLSX file on your computer (or the web).

You’ll practice your importing skills on a dataset containing weights of chickens and their feed type.

*** =instructions
- Load the `readxl` package.
- Use the `read_excel()` function to import `chickwts.xlsx`.  Call the resulting data frame `idp`.
- View the `head()` of `idp`.

*** =hint
Your call to `read_excel()` should take one argument: the name of the CSV file (in quotes).
...
*** =sct
```{r}
test_library_function("readxl")
test_function("read_excel")
...
salaries <- read_excel(___, sheet = ___)
...
# Import data: salaries
salaries <- read_excel(___, sheet = ___, range = ___)
...
test_function("read_excel", incorrect_msg = "Did you import your data with `read_excel` to create `trees`?", not_called_msg = "Don't forget to import your data with `read_excel` to create `trees`!")
...
test_error()
success_msg("Excellent work! The `read_excel()` function combined the three columns into one called `Girth; Height; Volume` because it couldn't recognize how the values were separated. If you see something like this in the future, now you know how to fix it!")
```
